...
class Observability::QueueUpdatesTest < ActiveSupport::TestCase
  ...
  test "enqueue claim finish retry and discard publish only after commit" do
    ...
    refute_includes broadcasts(Realtime::OperationsUpdates::STREAM).join, "PRIVATE_ERROR"
  end

  test "rollback and empty claim publish no updates" do
    assert_no_broadcasts(Realtime::OperationsUpdates::STREAM) do
  ...
  def assert_signal(&block)
    Realtime::OperationsUpdates.flush
    assert_changes -> { broadcasts(Realtime::OperationsUpdates::STREAM).size } do
      block.call
      Realtime::OperationsUpdates.flush
    end
    assert_includes broadcasts(Realtime::OperationsUpdates::STREAM).last, "operations_refresh"
    assert_includes broadcasts(Realtime::OperationsUpdates::STREAM).last, "queue"
  end
end
