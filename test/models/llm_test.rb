...
class LlmTest < ActiveSupport::TestCase
  test "без настроек запрос запрещён до обращения к сети" do
    assert_raises(Anyway::Config::ValidationError) do
      Llm.build_chat(settings: LlmConfig.new(provider: nil, model: nil, api_key: nil))
    end
  end

  test "частичная конфигурация и неверные значения отклоняются" do
  ...
  test "диалоги изолированы, модель проверяется по локальному реестру" do
  ...
  test "ошибка провайдера не скрывается и запрос не повторяется" do
    ...
    assert_raises(RubyLLM::ServiceUnavailableError) { Llm.build_chat(settings: settings).ask("Проверка") }
