...
## 2026-09-19 — админка и автоматический мониторинг

- Общая боковая навигация, компактный обзор очереди, единое оформление Mission Control и AgentPrism.
- Обновление каждые 5 секунд: Turbo morph для HTML, polling для AgentPrism; сохраняются ввод и выбранная вкладка, при отзыве роли панель очищается.
- `bin/dev` поднимает Grafana, Prometheus, Loki и Alloy. Логи доступны через Grafana; контейнеры локальны, production-доступ и оповещения настраиваются отдельно.
- Полный локальный CI: 134 Rails-теста / 694 проверки; браузер — 13 / 80; AnyCable — 12, imgproxy — 10, k6 — 44. TypeScript, 6 JS-тестов и production assets прошли. После последней правки оформления админки повторена узкая браузерная проверка.
- Конфиги Loki/Alloy и Prometheus/alerts проверены штатными валидаторами. Локально подтверждены четыре scrape targets и доставка JSON-логов в Loki.
