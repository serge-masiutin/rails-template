...
function App({ url }: { url: string }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!response.ok) throw new Error(`Не удалось загрузить трассы: HTTP ${response.status}`);
        const page = decodeTracePage(await response.json());
        setState({ status: "ready", ...page });
      } catch (error) {
        if (!controller.signal.aborted) setState({ status: "error", message: error instanceof Error ? error.message : "Ошибка загрузки трасс" });
      }
    }
    void load();
  ...
  return <>
    ...
    {state.status === "loading" && <p className="ops-message" role="status">Загрузка трасс…</p>}
    ...
    {state.status === "ready" && (state.data.length ?
      <div className="ops-viewer"><TraceViewer key={`${cursor}:${revision}`} data={state.data} /></div> :
      <p className="ops-message">Трасс пока нет. Они появятся после выполнения ApplicationAgent.</p>)}
  </>;
}
...
class ViewerBoundary extends Component<{ children: ReactNode }, { failed: boolean }> {
  ...
  render() {
    if (this.state.failed) return <p role="alert" className="ops-message">Не удалось отобразить трассу. Обновите страницу; если ошибка повторится, проверьте контракт AgentPrism.</p>;
    return this.props.children;
  }
}

const container = document.getElementById("agent-prism");
if (!container?.dataset.url) throw new Error("Не задан адрес трасс AgentPrism");
