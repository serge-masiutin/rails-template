...
- StarterApp uses Tailwind 4 with tokens in `app/assets/tailwind/application.css`, ERB/ViewComponent in `app/components`, and previews in `test/components/previews`. Apply the component/variant principles below in Ruby and ERB; see `Ui::NoticeComponent` for an existing variant map.
- Keep local Martian Mono through `app/assets/stylesheets/typography.css` and `shared/typography`; put UI text in `config/locales/en.yml`.
- Use the existing Herb tooling and `bin/rails tailwindcss:build`. Do not install Prettier, React, or a class-merging package solely to follow the JavaScript examples. Check narrow screens and the shared web/Android HTML.

# Tailwind CSS best practices

Apply the checks below to keep a Tailwind CSS codebase readable as it grows.
