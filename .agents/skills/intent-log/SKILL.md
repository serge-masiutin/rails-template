...
- Create or update a Russian intent log only when the user requests one. Use the requested location and preserve the existing format if a log is present.
- Use evidence from the current conversation and repository. The bundled extractor and hooks target Claude Code transcripts; do not assume they read Codex history or enable transcript staging as part of ordinary logging.
- Use `bin/gh` for GitHub queries. Run bundled tools only when their format matches the requested log; use the project wrapper for GitHub access. A log update does not itself authorize creating a PR, pushing, or merging; name a PR only when one exists.
