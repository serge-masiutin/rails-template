...
- StarterApp is a private Rails application; its root `package.json` has `private: true` and builds development tools and AgentPrism assets. This npm publishing workflow does not apply to ordinary dependency updates or Kamal deployments.
- Use [development](../../../docs/development.md) and [deployment](../../../docs/deployment.md) for those tasks. Apply the workflow below only to an actual npm package being released; use `bin/gh` for StarterApp GitHub operations and the project's pinned runtime versions.
