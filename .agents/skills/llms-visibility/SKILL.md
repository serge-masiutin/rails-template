...
- Apply this workflow only to public StarterApp content. Authentication and Action Policy remain required for account pages, admin panels, documents, and any Markdown representation; content negotiation must not expose a private representation.
- Use Rails routes/controllers and existing HTML content as the source; do not add a second frontend or publish private data in `llms.txt`. See [the HTTP contract](../../../docs/hotwire.md) and [architecture](../../../docs/architecture.md).

# Make a website visible to LLMs

Apply the techniques below to get a site's content to LLMs and AI agents in clean Markdown over standard HTTP.
