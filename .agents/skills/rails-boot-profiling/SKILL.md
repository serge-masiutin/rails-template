...
- Use Ruby and Bundler through `mise exec --` with the versions in `.ruby-version` and `Gemfile.lock`. `require-profiler` is not currently installed; make that prerequisite explicit before running its commands.
- Keep the target environment and cache conditions identical for before/after measurements. Preserve existing eager-load settings, Isolator, and thread-safety checks; do not disable project safeguards as a profiling fix.
- Keep generated profiles under `tmp/` and report measured results separately from hypotheses. See [testing](../../../docs/testing.md) for the distinct TestProf test-suite profiling workflow.
