class Observability::AgentTraceRecorder
  ...
  def record
    ...
    AgentTrace.connection_pool.with_connection { AgentTrace.create!(record_attributes) }
  rescue ActiveRecord::ActiveRecordError, InvalidTrace => error
    # A trace storage failure must not retry an already billed model request.
